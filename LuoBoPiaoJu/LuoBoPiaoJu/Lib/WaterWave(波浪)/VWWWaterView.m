...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _currentWaterColor = kNavBarColor;
...
//}
    float y = self.waveHeight;
    float Psx = self.frame.size.width;
//    NSLog(@"waterVavePs--%.2f", Psx);
    CGPathMoveToPoint(path, NULL, 0, y);
    ...
    //左上到右下颜色填充
    CGPathAddLineToPoint(path, nil, Psx, rect.size.height);
    //右上到左下颜色填充
    CGPathAddLineToPoint(path, nil, 0, rect.size.height);
    //暂未发现有什么用
    CGPathAddLineToPoint(path, nil, 0, self.waveHeight);
